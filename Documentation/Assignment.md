...
1. At what frequency does D1 toggle? [10000 Hz]

2. Do all LEDs toggle at *exactly* the same time? [While it looks like they all do, at a ceartin point no because they all have different interrupt]
...
1. What has happened to the speed of the timers? [The timers speed decreased when we change the APB prescale]

2. What is the new frequency of LED D1? [1250]

3. When we changed the frequency, did the Seven-Segment Light update rate change?  (hint, look at the clocks driving the APB1, APB2 buses and which timers are on which bus.  Recall that the Seven-Segment timer is Tim17) [No it did not since the internal clock division is turned off]
